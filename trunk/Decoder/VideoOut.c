...
#define SPACE_FOR_BUTTON    35
static void win_pos_change (vo_instance_t * instance_vo,int window)
{
    RECT rect_window;
    POINT point_window;
    HWND hwnd = (HWND)window;
    dx_instance_t * instance_dx = (dx_instance_t * )instance_vo;
  	/* update the window position and size */
	point_window.x = 0;
	point_window.y = 0;
	ClientToScreen (hwnd, &point_window);
	instance_dx->window_coords.left = point_window.x;
	instance_dx->window_coords.top = (point_window.y+SPACE_FOR_BUTTON);
	GetClientRect (hwnd, &rect_window);
	instance_dx->window_coords.right = rect_window.right + point_window.x;
	instance_dx->window_coords.bottom = (rect_window.bottom + point_window.y );

	/* update the overlay */
	if (instance_dx->overlay && instance_dx->display)
	    update_overlay (instance_dx);
...
static int set_window (vo_instance_t * instance, int window)
{
    dx_instance_t * instance_dx = (dx_instance_t *)instance;
    if(instance_dx->window == NULL)
    {
        instance_dx->window = (HWND)window;
        return 1;
    }
    return 0;
}
					       ...
					       uint8_t * const *, void * id))
{
    ...
    instance->vo.set_win = set_window;
    instance->vo.win_pos_chg = win_pos_change;
