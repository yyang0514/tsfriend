...
#define	mmx_r2r(op,regs,regd) \
        	_asm __volatile__ { #op " " #regs ", " #regd}


#define	emms() _asm __volatile__ {"emms"}

#define	movd_v2r(var,reg)	_asm __volatile__ { "movd " #var ", " #reg }
#define	movd_r2v(reg,var)	_asm __volatile__ {"movd " #reg ", " #var }

        /* AMD MMX extensions - also available in intel SSE */


#define mmx_m2ri(op,mem,reg,imm) \
        	_asm __volatile__ (#op " %1, %0, %%" #reg \
        			      : /* nothing */ \
        			      : "m" (mem), "i" (imm))

#define mmx_r2ri(op,regs,regd,imm) \
        	_asm __volatile__ (#op " %0, %%" #regs ", %%" #regd \
        			      : /* nothing */ \
        			      : "i" (imm) )

#define	mmx_fetch(mem,hint) \
        	_asm __volatile__ ("prefetch" #hint " %0" \
        			      : /* nothing */ \
        			      : "m" (mem))

#define	pmovmskb(mmreg,reg) \
        	_asm __volatile__ ("movmskps %" #mmreg ", %" #reg)

#define	sfence() _asm __volatile__ ("sfence\n\t")
